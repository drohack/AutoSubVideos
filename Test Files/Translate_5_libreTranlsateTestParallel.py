...
def translate_line(line: str, pbar) -> str:
    base_url = "http://localhost:4040/translate"

    params = {
        "q": clean_up_line(line),
        "source": "ja",
        "target": "en"
    }
...
def translate_subtitle_parallel(subtitle_file, output_file_path):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=num_workers) as executor:
        ...
        with tqdm(total=len(text_lines), unit="line", position=0, leave=True) as pbar:
            ...
            translated_lines = list(executor.map(lambda line: translate_line(line, pbar), text_lines))
...
en_subtitle_path = directory_path + "/" + os.path.splitext(os.path.basename(sub_path))[0] + ".libre.srt"
