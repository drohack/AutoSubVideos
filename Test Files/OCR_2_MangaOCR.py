...
MIN_FRAME_LENGTH_SEC = 0.4      # Minimum duration we keep text boxes (we throw out what's lower than this) (seconds * fps)
...
def ocr_video(video_file_path):
        ...
        while cap.isOpened():
            ...
            if ret:
                if i % 3 == 0:  # process every 3 frames
                    ...
                    results = reader.readtext(gray_frame, paragraph=True)
                    frame_result_array = []
    ...
    ja_subtitle_path = (directory_path + "/" + os.path.splitext(os.path.basename(video_file_path))[0]
                        + ".textbox-en.srt")
    create_empty_srt_file(ja_subtitle_path)
    ...
    with open(ja_subtitle_path, 'w', encoding='utf-8') as f:
        for i in range(len(final_text_box_array)):
            ...
            f.write(f'{i+1}\n')
            f.write(f'{text_box[0]} --> {text_box[1]} X1:{text_box[2][0][0]} X2:{text_box[2][2][0]} Y1:{text_box[2][0][1]} Y2:{text_box[2][2][1]}\n')
            f.write('{\\an8}' + f'{text_box[3]}\n\n')
